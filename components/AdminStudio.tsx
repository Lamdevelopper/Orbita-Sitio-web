...
export function AdminStudio({ email }: { email: string }) {
  ...
  return <main className="admin-studio">
    <header><div><span className="eyebrow light">ORBITA Â· MESA EDITORIAL</span><h1>Administrar, editar y publicar.</h1><p>Sesion privada: {email}</p></div><div className="admin-header-actions"><a href="/admin/post">Importación automática</a><a href="/docs/Guia_entrega_articulos_Orbita.pdf" target="_blank" rel="noreferrer">Guia para importar</a><a href="/admin/analytics">Ver analytics</a></div></header>
    ...
    {tab === "articles" && <div className="admin-workspace">
      <section className="admin-list admin-queue"><span className="eyebrow">ORDEN DE PORTADA</span><h2>Articulos en CMS</h2>{cmsQueue.length ? cmsQueue.map((item) => <article key={item.id}><div><span className="admin-pill">{item.status}</span><h3>{item.title}</h3><p>{item.homepageSlot} Â· posicion {item.homepageRank}</p></div><div className="admin-actions"><button onClick={() => beginArticle(item)}>Editar</button><button onClick={() => updateArticle(item, { status: "published" })}>Publicar</button></div></article>) : <p>Todo esta publicado. Los nuevos borradores apareceran aqui hasta que los publiques.</p>}</section>
