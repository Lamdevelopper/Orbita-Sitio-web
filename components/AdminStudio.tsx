...
export function AdminStudio({ email }: { email: string }) {
  const [tab, setTab] = useState<"articles" | "authors" | "editions">("articles");
  ...
  return <main className="admin-studio">
    ...
    <div className="admin-tabs"><button className={tab === "articles" ? "active" : ""} onClick={() => setTab("articles")}>Articulos</button><button className={tab === "authors" ? "active" : ""} onClick={() => setTab("authors")}>Autores</button><button className={tab === "editions" ? "active" : ""} onClick={() => setTab("editions")}>Ediciones</button></div>
