...
export function AdminStudio({ email }: { email: string }) {
  ...
  async function migrateStatic() { setMigrating(true); try { const res = await fetch("/api/admin/seed", { method: "POST" }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setMessage("Migración completada: " + data.imported + " importados, " + data.skipped + " ya existían."); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "No se pudo migrar."); } finally { setMigrating(false); } }
  ...
  return <main className="admin-studio"><header><div><span className="eyebrow light">ÓRBITA · MESA EDITORIAL</span><h1>Publicar, ordenar y medir.</h1><p>Sesión privada: {email}</p></div><a href="/admin/post">Importar artículo →</a><a href="/admin/analytics">Ver analytics →</a></header>
    <div className="admin-tabs"><button className={tab === "articles" ? "active" : ""} onClick={() => setTab("articles")}>Artículos</button><button className={tab === "manage" ? "active" : ""} onClick={() => setTab("manage")}>Administrar</button><button className={tab === "editions" ? "active" : ""} onClick={() => setTab("editions")}>Ediciones</button></div>{message && <p className="admin-message">{message}</p>}
